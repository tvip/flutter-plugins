...
+ (FLTAbsolutePositionMessage *)fromMap:(NSDictionary *)dict {
  ...
  pigeonResult.textureId = GetNullableObject(dict, @"textureId");
  NSAssert(pigeonResult.textureId != nil, @"");
  pigeonResult.absolutePosition = GetNullableObject(dict, @"absolutePosition");
  return pigeonResult;
}
- (NSDictionary *)toMap {
  return [NSDictionary dictionaryWithObjectsAndKeys:(self.textureId ? self.textureId : [NSNull null]), @"textureId", (self.absolutePosition ? self.absolutePosition : [NSNull null]), @"absolutePosition", nil];
}
@end
...
- (nullable id)readValueOfType:(UInt8)type 
{
  switch (type) {
    case 128:     
      return [FLTAbsolutePositionMessage fromMap:[self readValue]];
...
{
  if ([value isKindOfClass:[FLTAbsolutePositionMessage class]]) {
    [self writeByte:128];
...
void FLTAVFoundationVideoPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FLTAVFoundationVideoPlayerApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.AVFoundationVideoPlayerApi.absolutePosition"
        binaryMessenger:binaryMessenger
        codec:FLTAVFoundationVideoPlayerApiGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(absolutePosition:error:)], @"FLTAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(absolutePosition:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTTextureMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        FLTAbsolutePositionMessage *output = [api absolutePosition:arg_msg error:&error];
        callback(wrapResult(output, error));
      }];
    }
    else {
