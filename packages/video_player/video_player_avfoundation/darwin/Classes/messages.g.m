...
@interface FVPAbsolutePositionMessage ()
+ (FVPAbsolutePositionMessage *)fromList:(NSArray *)list;
+ (nullable FVPAbsolutePositionMessage *)nullableFromList:(NSArray *)list;
- (NSArray *)toList;
@end
...
@implementation FVPAbsolutePositionMessage
+ (instancetype)makeWithTextureId:(NSInteger )textureId
    absolutePosition:(nullable NSNumber *)absolutePosition {
  FVPAbsolutePositionMessage* pigeonResult = [[FVPAbsolutePositionMessage alloc] init];
  pigeonResult.textureId = textureId;
...
+ (nullable FVPAbsolutePositionMessage *)nullableFromList:(NSArray *)list {
  return (list) ? [FVPAbsolutePositionMessage fromList:list] : nil;
}
- (NSArray *)toList {
  return @[
    @(self.textureId),
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    case 128: 
      return [FVPAbsolutePositionMessage fromList:[self readValue]];
    case 129: 
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[FVPAbsolutePositionMessage class]]) {
    [self writeByte:128];
...
void SetUpFVPAVFoundationVideoPlayerApi(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FVPAVFoundationVideoPlayerApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.video_player_avfoundation.AVFoundationVideoPlayerApi.absolutePosition"
        binaryMessenger:binaryMessenger
        codec:FVPAVFoundationVideoPlayerApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(absolutePosition:error:)], @"FVPAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(absolutePosition:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FVPTextureMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        FVPAbsolutePositionMessage *output = [api absolutePosition:arg_msg error:&error];
        callback(wrapResult(output, error));
      }];
    } else {
      [channel setMessageHandler:nil];
